...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    ...
    httpOnly: true //prevent cross-site cripting
  };
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError("You are not loggrd in! Please loggin to get access", 401)
    );
  }
...
exports.forgetPassword = catchAsync(async (req, res, next) => {
  ...
  const message = `Forgot your password? submit a patch request with your new password abd passwordConfirm to ${resetURL}.\nIf you don't forget your password,Please ignore this email`;
  try {
    await sendEmail({
      email: "phone.madridista@gmail.com",
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError("Token is invalid or has expiresd", 400));
  }
